...
//    private static int traceCount = 0;
    ...
    FireworksCircleGraphics(Context context) {
        ...
        lineDecayRate = DisplayUtil.dip2px(context, LINE_DECAY_RATE);
        lineRectF = new RectF(0, 0, 0, 0);

        /* 星星大小 px */
        float starSize = DisplayUtil.dip2px(context, STAR_SIZE);
        starMaxVx = DisplayUtil.dip2px(context, STAR_MAX_VX);
        starMaxVy = DisplayUtil.dip2px(context, STAR_MAX_VY);
        starDecayRate = DisplayUtil.dip2px(context, STAR_DECAY_RATE);
        starDecayRateConst = DisplayUtil.dip2px(context, STAR_DECAY_RATE_CONST);
        starDisappearDistance = DisplayUtil.dip2px(context, STAR_DISAPPEAR_DISTANCE);
        ...
        linePaint.setStrokeWidth(DisplayUtil.dip2px(context, LINE_SIZE));
...
//        starPaint.setAntiAlias(true);
    ...
    void draw(Canvas canvas) {
        ...
        if (needRefresh) {
            circleX = (int) (width * MiSportConnectView.START_CIRCLE_X_SCALE);
            circleY = (int) (height * MiSportConnectView.START_CIRCLE_Y_SCALE);
