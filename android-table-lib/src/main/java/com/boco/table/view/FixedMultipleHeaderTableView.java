...
public class FixedMultipleHeaderTableView extends LinearLayout {
    ...
    public FixedMultipleHeaderTableView(Context context) {
        this(context, null);
    }

    public FixedMultipleHeaderTableView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FixedMultipleHeaderTableView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//                        refreshTable();
        ...
        for (int i = 0; i < mainTitles.length; i++) {
            if (viceTitles.get(mainTitles[i]) == null) {
                headerList.add(mainTitles[i]);
            } else {
                headerList.addAll(viceTitles.get(mainTitles[i]));
            }
        ...
        if (isTransverseScroll && isSelfAdaption) {
            cellWidths = new int[titlesAll.length];
            ...
            for (int i = 0; i < titlesAll.length; i++) {
                ...
                int wdith = (int) textPaint.measureText(titlesAll[i]);
            ...
            float surplusWidth = TableHeaderWidth +
                    TableSplitLineSize * (titlesAll.length + 1);//多余宽度
            ...
            if (textSumWidth < scrollWidth) {
                int addWidth = (int) ((scrollWidth - textSumWidth) / titlesAll.length);
                for (int i = 0; i < cellWidths.length; i++) {
        ...
        for (int m = 0; m < mainTitles.length; m++) {
            ...
            if (viceTitles.get(mainTitles[m]) == null) {
                titles = new String[]{mainTitles[m]};
            } else {
                List<String> titleList = new ArrayList<>();
                titleList = viceTitles.get(mainTitles[m]);
            ...
            LinearLayout.LayoutParams parentLayoutParams = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
            parentLayout.setOrientation(LinearLayout.VERTICAL);
            parentLayout.setLayoutParams(parentLayoutParams);

            LinearLayout viceLayout = new LinearLayout(context);
            ...
            if (titles.length == 1) {
                ...
                if (isCustomClolor) {
                    String[] textAndColor = (mainTitles[m] == null || "".equals(mainTitles[m])) ? new String[]{"", ""} : mainTitles[m].split(delimiter);
                    if (textAndColor.length > 0) {
                ...
                mainLayout.addView(childTv, childTVParams);
            }
...
//                    layout.setPadding((int) getResources().getDimension(R.dimen.table_padding), 0, (int) getResources().getDimension(R.dimen.table_padding), 0);
                ...
                if (titles.length >= 2 &&i < titles.length - 1) {
                    ...
                    viceLayout.addView(divider, divierParams);
                }
            }
            ...
            if (titles.length>1){
                ...
                View divider = new View(context);
                divider.setBackgroundColor(TableSplitLineColor);
                LayoutParams divierParams = new LayoutParams(LayoutParams.MATCH_PARENT,
                        TableSplitLineSize);
                parentLayout.addView(divider, divierParams);
            }

            LinearLayout.LayoutParams viceParams = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
            viceLayout.setLayoutParams(viceParams);
            viceLayout.setOrientation(LinearLayout.HORIZONTAL);
            parentLayout.addView(viceLayout);
    ...
    private class InitTableStruct extends AsyncTask<Void, Void, PageData> {
        ...
        protected PageData doInBackground(Void... params) {
            ...
            pageData.setViceHeaderData(adapter.getViceColHeaders());
